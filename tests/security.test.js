 ...
 * - Security utilities
 */
import { validateContentSize, validateExpiration, validateViewCount, validatePassword, isValidUTF8 } from '../src/core/validators/index.js';
import { secureClear, secureClearBuffer, getSafeErrorMessage, ERROR_MESSAGES, deriveKeyFromPassword, generateSalt, encryptWithPassword, decryptWithPassword, } from '../src/security.js';
// ============================================================================
// VALIDATION TESTS
// ============================================================================
...
describe('decryptWithPassword', () => {
    it('should decrypt with correct password', async () => {
        ...
        const decrypted = await decryptWithPassword(encrypted.encryptedData, password, encrypted.salt, encrypted.iv);
        expect(decrypted).toBe(original);
    });
    it('should fail with wrong password', async () => {
        const original = 'Secret message';
        const encrypted = await encryptWithPassword(original, 'TestPassword123');
        await expect(decryptWithPassword(encrypted.encryptedData, 'WrongPassword', encrypted.salt, encrypted.iv)).rejects.toThrow();
    });
    it('should handle empty content', async () => {
        const original = '';
        const password = 'TestPassword123';
    ...
    it('should fail with corrupted ciphertext', async () => {
        ...
        await expect(decryptWithPassword(corrupted, 'TestPassword123', encrypted.salt, encrypted.iv)).rejects.toThrow();
    });
    it('should fail with corrupted IV', async () => {
        ...
        await expect(decryptWithPassword(encrypted.encryptedData, 'TestPassword123', encrypted.salt, corruptedIv)).rejects.toThrow();
    });
    it('should fail with corrupted salt', async () => {
        ...
        await expect(decryptWithPassword(encrypted.encryptedData, 'TestPassword123', corruptedSalt, encrypted.iv)).rejects.toThrow();
    });
});
// ============================================================================
// INTEGRATION TESTS
