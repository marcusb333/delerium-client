 ...
 * on the view.html page when a delete token exists in localStorage.
 *
 * Test Coverage:
 * - Destroy button appears when delete token exists in localStorage
...
test.describe('View Page - Destroy Paste Button', () => {
    ...
    test('should show destroy button when delete token exists in localStorage', async ({ page, context }) => {
        // Set delete token in localStorage before navigating
        await context.addInitScript(() => {
            localStorage.setItem('deleteToken_test-paste-id', 'test-delete-token-789');
        });
    ...
    test('should successfully delete paste when destroy button is clicked', async ({ page, context }) => {
        // Set delete token in localStorage
        await context.addInitScript(() => {
            localStorage.setItem('deleteToken_test-paste-id', 'test-delete-token-789');
        });
        ...
        const tokenInStorage = await page.evaluate(() => {
            return localStorage.getItem('deleteToken_test-paste-id');
        });
...
//# sourceMappingURL=view-paste-destroy.spec.js.map